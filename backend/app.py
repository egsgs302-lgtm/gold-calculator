...
def get_gold_price(date: str, amount_try: float):
    try:
        ...
        # 1. Gold price in USD per ounce (XAU/USD)
        gold = yf.Ticker("GC=F")  # Gold Futures
        gold_data = gold.history(start=dt.strftime("%Y-%m-%d"), end=(dt + timedelta(days=1)).strftime("%Y-%m-%d"))
        if gold_data.empty:
        ...
        usdtry_data = usdtry.history(start=dt.strftime("%Y-%m-%d"), end=(dt + timedelta(days=1)).strftime("%Y-%m-%d"))
